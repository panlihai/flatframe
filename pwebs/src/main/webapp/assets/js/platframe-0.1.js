	...
	for (var i = 0; i < data.length; i++) {
		...
		if (obj.MENUTYPE == 'MENU') {// 如果是菜单的时候
			...
			for (var j = 0; j < chliddata.length; j++) {// 重新循环
				var objChild = chliddata[j].values;
				// 第一个菜单的第一个子应用程序
				if (firstAppId.length == 0) {
			...
			menuContent += '<li><a href="#"'
					+ ' onclick="clickListMenu(\'\',\'' + obj.MENUID
					+ '\',\'\',\'\',1,\'\',\'listView\',\''
					+ data[i].values.APPID + '\');"><i class="'
					+ data[i].values.MENUICON + '"  ></i>&nbsp;'
					+ data[i].values.MENUNAME + '</a></li>';
		}
	}
	...
	$
			.ajax({
				...
				success : function(data) { // 若Ajax处理成功后返回的信息
					...
					// APPFIELDS
					P_APPFIELDS = data.P_APP.values.P_APPFIELDS;
					// 设置标题
					$('#' + page + 'page_content_title').html(
							data.P_APP.values.APPNAME);
					// 显示列表按钮及表头
					...
					// 绑定查询事件
					$('#' + page + menuId + mainAppId)
							.click(
									function(e) {
										var searchValues = '';
										for (var j = 0; j < data.P_APP.values.P_APPFIELDS.length; j++) {
											var field = data.P_APP.values.P_APPFIELDS[j].values;
											if (field.ENABLESEARCH == 'Y') {
												...
												if (undefined != valueObj) {
													...
													if(field.DBTYPE!='STR'){
														//区间的查询条件
														var endValue = document.getElementById(field.APPID+ '.'+ field.FIELDCODE+'END');
														if(endValue!=undefined){
															value+='","'+field.FIELDCODE+'END":"'+endValue.value;
														}														
													}
												} else {
													var arrayObj = document.getElementsByName(field.APPID+ '.'+ field.FIELDCODE);
					...
					$("#" + page + "page_content_tbody tr").dblclick(
							function(e) {
								...
								listEdit(page, menuId, mainApp, mainAppId,
										pageFilter, P_APP.values.APPID, $obj
												.prop("id"));
							});
					// 分页内容生成
...
function createSearchFields(page, menuId, mainApp, mainAppId, pageFilter, data) {
	// 显示字段列表
	var contentFieldsResult = data.P_APP.values.P_APPFIELDS;
	var cols = 0;
	for (var i = 0; i < contentFieldsResult.length; i++) {
		if (contentFieldsResult[i].values.SHOWLIST == 'Y') {
			// 计算有几列
			cols++;
		}
	}
	...
	for (var i = 0; i < contentFieldsResult.length; i++) {
		var field = contentFieldsResult[i].values;
		if (field.ENABLESEARCH == 'Y') {
	...
	if (hsFilter) {
		...
		content += '&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default" onClick=""><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span> 重置</button></div></div>'
		content += '</form></th></tr>';
		return content;
	} else {
		return '';
...
function createListBtns(page, menuId, mainApp, mainAppId, pageFilter, data) {
	...
	// 按钮列表
	var btnList = data.P_APP.values.P_APPBUTTONS;
	if (btnList == undefined || btnList.length == 0) {
		return;
	}

	for (var i = 0; i < btnList.length; i++) {
		var obj = btnList[i].values;
		if (obj.BTNTYPE != 'LIST') {
			continue;
		}
		content += '<button type="button" class="btn btn-default" id="'
				...
				+ '\',\'' + obj.ACTCODE + '\',\''+pageFilter+'\');" ><span class="' + obj.BTNICON
	...
	for (var i = 0; i < contentFieldsResult.length; i++) {
		if (contentFieldsResult[i].values.SHOWLIST == 'Y') {
			page_content_header += '<th>'
					+ contentFieldsResult[i].values.FIELDNAME + '</th>';
		}
	}
 ...
 *            当前的动作是哪个页面 当为''时则表示为最上层的父级,否则为关联功能的ID
 */
function createListBody(page, menuID, mainApp, mainAppId, pageNumber, pageFilter, pageSize,
		data) {
...
function listAdd(page, menuId, mainApp, mainAppId, pageFilter, data) {
	// 设置标题
	$('#' + page + 'page_content_title').html(data.P_APP.values.APPNAME);
	// 生成表单按钮及表头
	$('#' + page + 'page_content_thead').html(
			createCardBtns(page, menuId, mainApp, mainAppId, data, pageFilter));
	// 生成表单主数据内容
	createCardBody(page, menuId, mainApp, mainAppId, data, pageFilter);
	// 生成表单主数据下方的内容
	$('#' + page + 'page_content_tfoot').html(
...
function listDelete(page, menuId, mainApp, mainAppId, appcode, actcode,
	...
	var content = '{\"' + this.P_APP.values.APPID + '\":{\"P_IDS\":\"\''
			+ idstr;
	content += '\'\"}}';
	if (id_array.length != 0) {
		$.ajax({
			...
			success : function(data) { // 若Ajax处理成功后返回的信息
				...
				// 分页内容生成
				$('#' + page + 'page_content_tfoot').html('');
				createList(page, mainApp, mainAppId, P_PAGENUM, pageFilter,
						'listView', appcode)
			},
			error : function(msg) { // 若Ajax处理失败后返回的信息
				alert("正在开发中...");
...
function cardSave(page, menuId, mainApp, mainAppId, actType, pageFilter) {
	...
	var err = '';
	var appfields = this.P_APP.values.P_APPFIELDS;
	// 内容数据
	var content = '{' + this.P_APP.values.APPID + ':{';
	// 
	for (var j = 0; j < appfields.length; j++) {
		var field = appfields[j].values;
				...
				.ajax({
					...
					success : function(data) { // 若Ajax处理成功后返回的信息
						...
						case 'cardSaveCopy':// 需要把ID值清空
							data.P_CARDVALUE.values.ID = '';// 需要把其它默认值也赋值为空
							// 把默认值清空
							for (var i = 0; i < data.P_APP.values.P_APPFIELDS.length; i++) {
								// 获取字段配置信息
								var field = data.P_APP.values.P_APPFIELDS[i].values;
								if (field.FIELDDEFAULT.length > 0
										|| field.AUTOCODE.length > 0) {
									data.P_CARDVALUE.values[field.FIELDCODE] = '';
								}
							}
							listAdd(page, menuId, mainApp, mainAppId,
									pageFilter, data);
...
function createCardBody(page, menuId, mainApp, mainAppId, data, pageFilter) {
	if (data.P_APP.values.CARDPAGE == undefined
			|| data.P_APP.values.CARDPAGE.length == 0) {
		...
		$('#' + page + 'page_content_tbody').html(
				createFormByFieldsConfig(page, menuId, mainApp, mainAppId,
						data, data.P_APP.values.P_APPFIELDS, val));
		// 对关联功能产生列表内容事件
		$(function() {
			var name = '#' + data.P_APP.values.APPID + 'CARDTABS a';// 表单TAB
			$(name + '[data-toggle="tab"]').on(
					...
					function(e) {
						...
						var itemAppId = activeTabHref.substring(
								1 + data.P_APP.values.APPID.length,
								activeTabHref.length - 7);
						// 如果是点击的是本身的tab页则不处理
						if (itemAppId.length != 0
								&& itemAppId != '#' + data.P_APP.values.APPID
										+ 'CARDTAB') {
							// 从服务器获取内容
							createList(data.P_APP.values.APPID + itemAppId
									+ 'child', menuId, data.P_APP.values.APPID,
		...
		var page = CTX + '/assets/templates/' + data.P_APP.values.CARDPAGE;
...
function createFormByFieldsConfig(page, menuId, mainApp, mainAppId, data,
	...
	if (undefined != itemAppArray && itemAppArray.length > 0) {
		...
		for (var i = 0; i < itemAppArray.length; i++) {
			var itemApp = itemAppArray[i].values;
...
function createCardFormBy(page, menuId, mainApp, mainAppId, data, fieldsList,
	...
	while (i < fieldsList.length) {
		if (fieldsList[i].values.SHOWCARD == 'Y') {
			content += '<div class="form-group" style="width:99%;margin-left:15px">';
			content += createLabel(fieldsList[i].values.APPID + '.'
					+ fieldsList[i].values.FIELDCODE,
					fieldsList[i].values.FIELDNAME);
			content += ''
					+ createCardFieldInput(page, menuId, mainApp, mainAppId,
							data, fieldsList[i].values, values, false);
			content += "</div>";
		} else {
			if (values != undefined
					&& values[fieldsList[i].values.FIELDCODE] != undefined) {
				hcontent += '<input id="' + fieldsList[i].values.APPID + '.'
						+ fieldsList[i].values.FIELDCODE
						+ '" type="hidden" value="'
						+ values[fieldsList[i].values.FIELDCODE] + '" />';
			} else {
				hcontent += '<input id="' + fieldsList[i].values.APPID + '.'
						+ fieldsList[i].values.FIELDCODE
						+ '" type="hidden" value="" />';
...
function findDicByFieldName(data, dicId) {
	// 应用程序中获取数据字典列表
	var dicList = data.P_APP.values.P_APPDICS;
	for (var i = 0; i < dicList.length; i++) {
		var dic = dicList[i].values;
		if (dic.DICID == dicId) {
			return dic;// 获取静态字典
		}
	}
...
function createFieldQuerySelect(page, menuId, data, field, value) {
	...
	var dicObj = dicArray.length > 0 ? dicArray[0].values : '';
	// 处理字典赋值列表
	var fields = '';
	...
	for (var i = 0; i < dicAppDetails.length; i++) {
		var appDetail = dicAppDetails[i].values;
...
function createListModal(page, menuId, mainAppId, pageNumber, action, appCode,
	...
	$.ajax({
		...
		success : function(data) { // 若Ajax处理成功后返回的信息
			...
			// 分页内容生成
			$('#' + page + 'page_content_tfoot').html(
					createListModalPaging(page, menuId, mainAppId, fields,
							value, data, action, appCode, pageNumber));
			$('#' + page + 'page').modal({
				show : true
			});
		},
...
function onSelectedFromListModal(page, menuId, mainAppId, fields, value, data,
	...
	for (var i = 0; i < valueObjs.length; i++) {
		var valueObj = valueObjs[i].values;
		if (id == valueObj.ID) {
