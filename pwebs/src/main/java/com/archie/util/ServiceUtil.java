...
public class ServiceUtil {
	 ...
	 * @return
	 */
	public static DynaBean getDefaultFieldValueByLinkFilter(DynaBean mainBean, String itemAppId, List<DynaBean> fieldBeanList,
			String filter) {
		...
		for (String fld : flds) {
			// 获取需要初始化的子表字段
			for (DynaBean fieldBean : fieldBeanList) {
	...
	public static List<DynaBean> getFieldBeanByAppBean(DynaBean appBean,DynaBean defBean){
		...
		for (DynaBean bean : list) {
			...
			fieldBean.setStr("FIELDNAME", bean.getStr("FIELD_DES", bean.getStr("FIELD_CODE")));
