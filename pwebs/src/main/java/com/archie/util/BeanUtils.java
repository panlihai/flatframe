...
public class BeanUtils {
	 ...
	 * @return
	 */
	public static List listMapToDynaBean(List<Map> list) {
	 ...
	 * @param appBean
	 * @param saveBean
	 * @param iscover 是否覆盖已经有的值
	 */
	public static void setDefaultValueByAppFieldSetting(DynaBean appBean, DynaBean saveBean,boolean iscover) {
		List<DynaBean> fieldsList = (List<DynaBean>) appBean.get(PageUtil.PAGE_APPFIELDS, new ArrayList<DynaBean>());
		for (DynaBean field : fieldsList) {
			...
			case KEY_TABLEFIELDDATE_TIME:
				saveBean.setStr(field.getStr("FIELDCODE"), DateUtils.getStringFromDate(KEY_TABLEFIELDDATE_TIME));
				break;
			case KEY_TABLEFIELDDATE:
				saveBean.setStr(field.getStr("FIELDCODE"), DateUtils.getStringFromDate(KEY_TABLEFIELDDATE));
				break;
			}
